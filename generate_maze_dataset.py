#!/usr/bin/env python3
'''
generate_maze_dataset.py - 生成迷宫数据集
...
# 迷宫参数
MAZE_SIZE_METERS = 32.0  # 迷宫实际大小(米)，与BreezySLAM默认值匹配
WALL_THICKNESS_METERS = 0.5  # 墙壁厚度(米)

# 数据集参数
NUM_SCANS = 800  # 扫描数量
SCAN_SIZE = 682  # 每次扫描的点数 (与BreezySLAM的MinesLaser/URG04LX兼容)
...
def generate_dataset(output_file, maze_size_pixels=800, wall_thickness_pixels=20):
    ...
    # 缩放因子：从像素坐标转换为米
    scale_factor = MAZE_SIZE_METERS / maze_size_pixels
