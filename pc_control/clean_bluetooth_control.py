...
# 基础配置：完全使用英文，避免中文字体问题
matplotlib.rcParams['font.family'] = 'DejaVu Sans'
matplotlib.use('TkAgg')

class CleanBluetoothControl:
    def __init__(self):
    ...
    def setup_gui(self):
        """设置图形界面"""
        self.root = tk.Tk()
        self.root.title("Clean Bluetooth Control - Robot System")
    ...
    def setup_radar_panel(self, parent):
        ...
        self.ax.set_ylabel('Distance (m)', labelpad=20, fontsize=10)
        self.ax.set_title(
            "Robot Radar Real-time Scan (50ms/frame | Mouse wheel zoom)",
        ...
        def on_scroll(event):
            ...
            self.log_data(f"[Zoom] Distance range: 0 ~ {new_max:.2f}m")
        ...
        if angle_matches:
            ...
            for axis_name, value_str in angle_matches:
                try:
                    ...
                    self.log_data(f"[Angle] {axis_name}: {temp_angles[axis_name]:.2f}°")
                    parsed_flag = True
                except Exception as e:
                    self.log_data(f"[Angle Error] {axis_name}:{value_str} | Error: {e}")
            ...
            for angle_str, dist_str, quality_str in radar_matches:
                try:
                    ...
                    if quality > 0 and 50 <= distance_m <= 12000:
                        ...
                        self.log_data(f"[Radar] Angle: {angle_deg:.1f}° | Distance: {distance_m:.3f}m | Quality: {quality}")
                        parsed_flag = True
                except Exception as e:
                    self.log_data(f"[Radar Error] {angle_str},{dist_str},{quality_str} | Error: {e}")
            ...
            for i, (angle, dist) in enumerate(combined):
                ...
                if len(neighbors) >= 2:
                    ...
                    if diff_from_mean > std_factor * neighbor_std and all_neighbors_exceed:
                        is_abnormal = True
                        self.log_data(f"[Filter] Angle: {np.degrees(angle):.1f}° | Distance: {dist:.3f}m")
        ...
        if motor_matches:
            for rpmA_str, rpmB_str in motor_matches:
                try:
                    ...
                    self.log_data(f"[Motor] A={rpmA:.1f}, B={rpmB:.1f}")
                    parsed_flag = True
                except Exception as e:
                    self.log_data(f"[Motor Error] {rpmA_str},{rpmB_str} | Error: {e}")
        ...
        if imu_matches:
            for accX, accY, accZ, gyroX, gyroY, gyroZ in imu_matches:
                try:
                    ...
                    self.log_data(f"[IMU] Acc={accel}, Gyro={gyro}")
                    parsed_flag = True
                except Exception as e:
                    self.log_data(f"[IMU Error] {accX},{accY},{accZ},{gyroX},{gyroY},{gyroZ} | Error: {e}")
        
        # 缓冲区过长时清理
        if not parsed_flag and len(self.data_buffer) > 600:
            self.log_data(f"[Buffer Clean] Discard first 500 chars")
    ...
    def serial_read_thread(self):
        ...
        try:
            ...
            self.log_data(f"✅ Successfully opened port: {self.COM_PORT} (Baudrate: {self.BAUDRATE})")
            self.connected = True
            ...
            while self.running and self.connected:
                if self.serial_port.in_waiting > 0:
                    ...
                    if line:
                        ...
                        self.log_data(f"[Serial Data] Received: {line[:50]}")
                        self.parse_buffer_data()  # 解析数据
                time.sleep(0.01)  # 降低CPU占用
                
        except serial.SerialException as e:
            self.log_data(f"❌ Serial Exception: {e}")
            self.connected = False
            self.status_label.config(text="Disconnected", foreground="red")
            self.connect_btn.config(text="Connect")
        except Exception as e:
            self.log_data(f"❌ Data Reading Exception: {e}")
        finally:
            if self.serial_port and self.serial_port.is_open:
                self.serial_port.close()
                self.log_data("📌 Serial port safely closed")
    ...
    def update_radar_plot(self, frame, ax, radar_line, angle_text, manual_zoom_flag):
        ...
        # 2. 角度文本更新 - 使用英文
        angle_text.set_text(
            "Robot Three-Axis Angles (Real-time)\n"
            f"Roll: {self.axis_angles['roll']:.2f}°\n"
            f"Pitch: {self.axis_angles['pitch']:.2f}°\n"
            f"Yaw: {self.axis_angles['yaw']:.2f}°"
        )
