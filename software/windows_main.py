...
from modules.communication import WindowsRobotInterface
...
class WindowsRobotNavigationSystem:
    """Windows优化的机器人导航系统"""
    ...
    def __init__(self, config_file: str = "config.json"):
        ...
        logger.info("Windows机器人导航系统已初始化")
    
    def _apply_windows_settings(self):
        """应用Windows特定设置"""
        # 设置matplotlib后端（Windows优化）
        try:
            import matplotlib
            matplotlib.use('TkAgg')  # Windows友好的后端
        except Exception as e:
            logger.debug(f"设置matplotlib后端失败: {e}")
        ...
        try:
            ...
            if hasattr(current_thread, 'set_priority'):
                current_thread.set_priority(threading.HIGH_PRIORITY)
        except Exception as e:
            logger.debug(f"设置线程优先级失败: {e}")
    ...
    def _init_components(self):
        ...
        # 1. 通信接口 (Windows优化版本)
        try:
            self.robot_interface = WindowsRobotInterface(
            ...
            logger.info("Windows机器人接口初始化成功")
        except Exception as e:
            logger.error(f"机器人接口初始化失败: {e}")
            # 尝试使用传统接口作为备选
    ...
    def _show_connection_status(self):
        """显示连接状态信息"""
        if hasattr(self.robot_interface, 'get_connection_status'):
            status = self.robot_interface.get_connection_status()
            logger.info("=== 连接状态 ===")
            for key, value in status.items():
                logger.info(f"{key}: {value}")
            
            # 测试连接
            if hasattr(self.robot_interface, 'test_connection'):
                if self.robot_interface.test_connection():
                    logger.info("连接测试: 正常")
                else:
                    logger.warning("连接测试: 异常")
    ...
    def _main_loop(self):
        ...
        while self.is_running:
            try:
                ...
                # 定期检查连接状态
                if time.time() - last_status_check > 5.0:
    ...
    def _check_connection_health(self):
        ...
        try:
            if hasattr(self.robot_interface, 'test_connection'):
                if not self.robot_interface.test_connection():
                    logger.warning("连接健康检查失败，尝试重新连接...")
                    if hasattr(self.robot_interface, 'reconnect'):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Windows智能小车导航系统')
    ...
    parser.add_argument('--test', '-t',
                      action='store_true',
                      help='运行测试模式')
    parser.add_argument('--auto-detect', '-a',
                      action='store_true',
                      help='自动检测串口')
    ...
    system = WindowsRobotNavigationSystem(config_file)
